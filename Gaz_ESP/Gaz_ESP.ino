...
void komutGeldi(uint8_t src, uint8_t id, const uint8_t *data, uint8_t len, int8_t rssi) {
  ...
  Serial.printf("KOMUT ALINDI <- Gaz: %.1f%%\n", komut);
...
void setup() {
  ...
  Serial.print("Gaz ESP32 MAC: ");
  Serial.println(WiFi.macAddress());
  Serial.println("Gaz hazir, fake sinyal gonderiliyor...");
}
...
void loop() {
  ...
  // float gaz = (ham / 4095.0) * 100.0;
  float zaman = millis() / 1000.0;
  float gaz = (sin(zaman) + 1.0) * 50.0;

  protocol.sendReliable(1, 1, (uint8_t*)&gaz, sizeof(gaz));
  Serial.printf("Gonderildi -> Gaz: %.1f%%\n", gaz);
